...
import test.mypic.dto.MypicDto;
import test.util.DbcpBean;

public class MypicDao {
	private static MypicDao dao;
	private MypicDao() {}
	public static MypicDao getInstance() {
		if(dao==null) {
			dao=new MypicDao();
		}
		return dao;
	...
	//이미지 한개의 정보 리턴하는 메소드
	public MypicDto getData(int num) {
		MypicDto dto=null;
	...
	public boolean insert(MypicDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			...
			String sql = "INSERT INTO board_mypic"
					+ " (num, writer, caption, imagePath, regdate)"
					+ " VALUES(board_mypic_seq.NEXTVAL,?,?,?,SYSDATE)";
			pstmt = conn.prepareStatement(sql);
	...
	public List<MypicDto> getList(MypicDto dto){
		List<MypicDto> list=new ArrayList<MypicDto>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
