...
<body>
	...
	<p class="bbb">form에 submit 이벤트가 일어 났을때 폼전송을 막고 ajax 전송해보기</p>
	...
	<script>
		...
		.addEventListener("submit",function(event){
			...
			//폼에 입력한 이름과 주소 읽어오기
			//"input[name=name]" 은 input 요소이면서 name 속성의 값이 name인 요소 찾기
			let name=document.querySelector("input[name=name]").value;
			//"input[name=addr]" 은 input 요소이면서 name 속성의 값이 addr인 요소 찾기
			let addr=document.querySelector("input[name=addr]").value;
			// form의 action 속성의 value 읽어오기
